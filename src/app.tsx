...
function Chat() {
    ...
    onMessage: useCallback(
      (message: MessageEvent) => {
        try {
          ...
          } else if (data.type === "codex-review-complete") {
            toasts.add({
              title: `Review ${data.reviewId}: ${data.verdict} (${data.score}/100)`,
              description: data.summary,
              timeout: 8000
            });
          } else if (data.type === "codex-review-error") {
            toasts.add({
  ...
  const {
    ...
    onToolCall: async () => {
      // All tools in this agent run on the server.
    }
  });
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-lg font-semibold text-kumo-default">
              <span className="mr-2">⛅</span>Codex Review Agent
            </h1>
